  ...
  return (
    <>
      ...
      >
        <button
          onClick={() => {
            setCurrentDateObj(new Date(currentYear, currentMonth - 1, currentDay));
          }}
        >
          Prev
        </button>
      ...
      <div className="selectedRange" style={{
        display: "flex",
        flexDirection: "row",
        justifyContent: "center",
        gap: "20px"
      }}>
        {!startDate && <div><b>Please select start date and end date from the calendar</b></div>}
        {startDate && <div><b>Start Date:</b> {startDate.toDateString()}</div>}
        {endDate && <div><b>End Date:</b> {endDate.toDateString()}</div>}
      </div>
